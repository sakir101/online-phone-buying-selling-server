...
async function run() {
  ...
  const bookingCollection = client
    .db("mobileHunterUser")
    .collection("bookphone");
  const paymentsCollection = client
    .db("mobileHunterUser")
    .collection("payPrice");
  const reportingCollection = client
    .db("mobileHunterUser")
    .collection("reportPhone");
  ...
  try {
    ...
    app.get("/advertiseproductone/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: ObjectId(id) };
      const singleAdvertiseProduct = await advertiseCollection.findOne(query);
      res.send(singleAdvertiseProduct);
    });
    ...
    app.get("/allreports", verifyJWT, verifyAdmin, async (req, res) => {
      const query = {};
    ...
    app.get("/advertiseProduct", async (req, res) => {
      ...
      const productsFilter = advertiseProduct.filter(
        (product) => product.payment === "none"
      );
      res.send(productsFilter);
    });
    ...
    app.delete("/deleteReportAd/:id", async (req, res) => {
      const productID = req.params.id;
      const query = { productID };
      const result = await advertiseCollection.deleteOne(query);
      res.send(result);
